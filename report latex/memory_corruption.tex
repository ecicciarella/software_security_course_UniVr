...
\begin{itemize}
    \item \textbf{Analisi Statica}: L'attaccante analizza il codice macchina per ricostruire il \textit{Control Flow Graph} (CFG). In assenza di adeguate tecniche di \textit{Code Obfuscation}, il flusso di controllo risulta facilmente intelligibile tramite strumenti (es. IDA Pro o Ghidra).
    \item \textbf{Analisi Dinamica}: Eseguendo il programma in un debugger (es. GDB), l'attaccante osserva la ''semantica concreta'' del programma, monitorando l'allocazione delle variabili in memoria e i valori dei registri della CPU in tempo reale.
\end{itemize}
...
Il programma copiava una stringa di testo (il nome del cavallo del protagonista) dal file di salvataggio verso un buffer di dimensione fissa allocato sullo \textit{Stack}, senza eseguire alcun controllo sui limiti (termine tecnico: \textit{bounds checking}).\\
Questa tecnica di attacco, fornire in input una stringa intenzionalmente più lunga della capienza del buffer, è stata formalizzata per la prima volta nel 1996 nell'articolo di Aleph One pubblicato su \textit{Phrack Magazine}:
...
Alterando il \textit{Return Pointer}, l'attaccante è riuscito a dirottare il flusso di controllo verso un payload malevolo (\textit{Shellcode}) inserito all'interno del finto nome del cavallo.
